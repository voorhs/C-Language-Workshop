...
void Parser::gl() {
    ...
    if (curr_type == LEX_STRING)
        ...
        curr_bool_value = curr_lex.get_bool_value();
...
void Parser::D1() {
    ...
    while (curr_type == LEX_INT || curr_type == LEX_BOOL || curr_type == LEX_STRING) {
        D();
...
void Parser::D() {
    ...
    if (curr_type == LEX_ASSIGN) {
        ...
        if (curr_type == LEX_INT_CONST || curr_type == LEX_FALSE || curr_type == LEX_TRUE || curr_type == LEX_STRING_CONST)
            gl();
        else throw curr_lex;
    }
    ...
    while (curr_type == LEX_COMMA) {
        ...
        if (curr_type == LEX_ASSIGN) {
            ...
            if (curr_type == LEX_INT_CONST || curr_type == LEX_BOOL || curr_type == LEX_STRING_CONST)
                gl();
            else throw curr_lex;
        }
    }
...
void Parser::N() {
    while (curr_type == LEX_NOT || curr_type == LEX_PLUS )  // NOT, unary plus
        gl();
...
void Parser::F() {
    ...
    } else if (curr_type == LEX_INT_CONST || curr_type == LEX_BOOL || curr_type == LEX_STRING_CONST || curr_type == LEX_ID)
        gl();
    else throw curr_lex;
}
